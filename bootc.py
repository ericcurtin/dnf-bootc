...
class BootcPlugin(Plugin):
    ...
    def transaction(self):
        ...
        if actions:
            ...
            image_id = subprocess.run(['podman', 'images', '-q', 'localhost/os:latest'], capture_output=True, text=True, check=True)
            subprocess.run(['bootc', 'switch', '--transport', 'containers-storage', image_id.stdout.strip()], check=True)
